...
const visuals=fs.readFileSync('src/WorldVisuals.js','utf8');
...
ok('1 modulos do jogo presentes', ['GamePhysics.js','Profiler.js','Rodas.js','TestTrack.js','WorldLayout.js','WorldVisuals.js','main.js'].every(file=>files.includes(file)));
...
ok('4 mapa e apoio usam geometria compartilhada',track.includes('const layout=createLayout()')&&track.includes('return layout.groundHeight(x,z)')&&track.includes('buildWorldVisuals(group,layout'));
...
ok('6 curva valida OBB e usa yaw amortecido',track.includes('function obbIntersectsAABB')&&track.includes('function moveVehicle')&&track.includes('function vehicleTurnAllowed')&&main.includes('stabilizeYawRate')&&main.includes('track.vehicleTurnAllowed'));
...
ok('8 câmera e apoio anti-penetração preservados',track.includes('function cameraSafePosition')&&main.includes('track.cameraSafePosition')&&track.includes('function twoWheelPose')&&main.includes('track.twoWheelPose')&&track.includes('let y=-Infinity')&&main.includes("const turnLean=name==='moto'"));
ok('9 rodas, instancing, LOD e sombras',main.includes('distance/Math.max(.05,r.raio')&&main.includes('if(r.dianteira)r.pivo.rotation.y=steerVisual')&&visuals.includes('new THREE.InstancedMesh')&&visuals.includes('new THREE.LOD()')&&main.includes('renderer.shadowMap.enabled=true')&&!/EffectComposer|UnrealBloom|postprocessing/.test(main));
